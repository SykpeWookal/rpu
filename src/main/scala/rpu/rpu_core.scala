...
class rpu_core(thread_number: Int) extends Module {
  ...
  // thread control
  val tc = Module(new rpu_thread_control(thread_number))
  // pc group 
  val pcg = Module(new rpu_pc_group(thread_number))
  // register file group
  val regg = Module(new rpu_register_file_group(thread_number))
  // decoder
  ...
  regg.io.thread_id_we := tc.io.thread_id_we
  regg.io.thread_id_wdata := tc.io.thread_id_wdata
  ...
  regg.io.raddr_a := decoder.io.rs1 
  regg.io.raddr_b := decoder.io.rs2 
  regg.io.raddr_c := decoder.io.rs3 
  // ID/EX Stage Register
  when (tc.io.idex_clear) {
    idex_stage_regs := idex_stage_reset
  } .elsewhen (tc.io.idex_we) {
    idex_stage_regs.valid := ifid_stage_regs.valid
    idex_stage_regs.pc := ifid_stage_regs.pc
    idex_stage_regs.npc := ifid_stage_regs.npc
    idex_stage_regs.r1 := regg.io.rdata_a
    idex_stage_regs.r2 := regg.io.rdata_b
    idex_stage_regs.r3 := regg.io.rdata_c
  ...
  regg.io.we_a := mmwb_stage_regs.reg_write
  regg.io.waddr_a := mmwb_stage_regs.reg_write_addr
  regg.io.wdata_a := mmwb_stage_regs.npc
  when (mmwb_stage_regs.reg_write) {
    switch(mmwb_stage_regs.reg_write_src) {
      is (reg_w_src_t.mem) {
        regg.io.wdata_a := mmwb_stage_regs.mem_result
      }
      is (reg_w_src_t.alu) {
        regg.io.wdata_a := mmwb_stage_regs.alu_result
      }
      is (reg_w_src_t.pc4) {
        regg.io.wdata_a := mmwb_stage_regs.npc
      }
...
object rpu_core extends App {
  (new chisel3.stage.ChiselStage).emitVerilog(new rpu_core(4))
}